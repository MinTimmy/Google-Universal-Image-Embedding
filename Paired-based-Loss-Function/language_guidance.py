...
class LanguageGuide(torch.nn.Module):
    ...
    def regularize(self, batch, labels, sample_indices=None):
        ...
        if self.iter_count >= self.activation_iter:
            ...
            bsame_labels = (labels == labels.view(-1, 1)).to(batch.device).T
...
def relabel(model,
    ...
    crop_size = crop_im_size = [224, 224]
    ...
    for i, data_input in enumerate(iterator):
        ...
        with torch.no_grad():
            ...
            data_input = tfds.as_numpy(data_input)
            # print(data_input)
            images, labels = data_input
            images = torch.from_numpy(images)
            # print(images.shape)
            labels = torch.from_numpy(labels)
            # data_input[0] = torch.from_numpy(data_input[0])
            # for j in range(2):
            #     data_input[j] = torch.from_numpy(data_input[j])
            # input = data_input[0]
            # input = torch.from_numpy(image)
            images = torch.permute(images, (0,3,1,2))
            # print('images', images.shape)
            out = model(images.to(device))
        for idx, label in zip(out, labels.cpu().detach().numpy()):
